%% Compute kernels for constant growth, aggregation and breakage length-based
% Author = Plath, Timo
...
%                       moment
%         S_L           a vector containing the length-basewd source term 
%                       contributions to each of the moments, i.e. dM_k/dt = S_L

function [A1,A2,S_L] = GrowthAggregationBreakageLengthBased(xi_alpha,w_alpha,G,a,beta,b_alpha)
    % number of weights
    n = length(w_alpha);
    ...
    for k=0:2*n-1
        ...
        for i = 1:n
            for r = 1:n
                % length-based aggregation kernel
                A3(k+1,i) = A3(k+1,i) + (((xi_alpha(i)^3 + xi_alpha(r)^3)^(k/3)) * w_alpha(i) * w_alpha(r));
                A4(k+1,i) = A4(k+1,i) + (xi_alpha(i)^k * w_alpha(i) * w_alpha(r));
            end
        end
        % length-based breakage
        A5(k+1,:) = xi_alpha.^k;
